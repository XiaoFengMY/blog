...
const RegistForm = () => {
    ...
    const onFinish = (values) => {
        axios.post(BASE_URL + "/users/regist", values).then((res) => {
            if (res.status === 200) {
                if (res.data.code === 1) {
                    // 不能在组件销毁后设置state
                    navigate("/Login");
    ...
    Modal.info({
        ...
        content: (
            <div>
                ...
                <h6>第三条 各方的权利义务</h6>
                <p>3.1 用户的权利义务</p>
                <p>3.1.1用户应遵守中国有关的法律法规规定及本协议约定条款。</p>
                <p>
                ...
                <p>
                    6.1
                    本协议所指知识产权是指与平台服务相关的各类过去有效的、现行有效的、或即将产生的知识产权，包括但不限于商标、著作权、计算机软件、发明、实用新型、外观设计以及提出相关申请的权利。
                </p>
                <p>
                ...
                <p>
                    7.6本平台在为用户提供
                    信息存储空间服务的同时，十分重视互联网环境的绿色、健康以及其他用户的知识产权和其他合法权益的保护。用户在上传分享内容时，须注意不得侵犯任何个人、企事业单位或社会团体的合法权益,包括但不限于姓名权、名称权、名誉权、荣誉权、肖像权、隐私权、知识产权等权利，否则，用户应对其自身行为及对由此产生的任何后果承担全部法律责任。本平台仅为用户提供上传服务供用户分享内容，本平台对用户上传的内容不做任何修改或编辑，本平台坚决反对任何违反中华人民共和国法律法规的行为。
                </p>
                <p>
